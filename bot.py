...
# ============================================================
BOT_TOKEN = "8620265232:AAGobdimWUiCw-7jGbkc-ge1SQiFKMQUS7k"  # Set your BotFather token here; no environment variable is used.
ADMIN_ID = 7709767483
DB_FILE = "bot.sqlite"
...
def reply_button(text):
    # Regular reply-keyboard KeyboardButton objects do not support a "style"
    # (color) parameter in python-telegram-bot -- only InlineKeyboardButton
    # does. Passing style here would raise a TypeError.
...
def admin_plan_menu():
    ...
    for category, label in (("choose", "══════« CHOSE PLAN »═════"), ("pro", "══════« PRO PLAN »═════")):
        ...
        for p in category_plans:
            buttons.append([style_button(f"💎 {p['name']}", f"admin:plan:view:{p['id']}", style="primary")])
            buttons.append([
...
async def user_demo(q, context):
    ...
    if not rows:
        await context.bot.send_message(
            ...
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup([[
                style_button("💎 CHOOSE PLAN", "user:premium", style="success")
            ]]),
        )
        return
