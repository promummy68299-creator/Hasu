...
def plan_payment_keyboard(plan_id, plan_link=None):
    ...
    # Only add link button if link is a valid HTTP/HTTPS URL
    if plan_link and isinstance(plan_link, str):
        plan_link = plan_link.strip()
        if plan_link.startswith(('http://', 'https://')):
            kb.add(types.InlineKeyboardButton("🔗 Open Plan Link", url=plan_link))
        else:
            logger.warning(f"Invalid plan link for plan {plan_id}: {plan_link}")
    ...
    if data == "get_premium":
        ...
        try:
            ...
            caption = (f"📦 Plan: {plan['name']}\n💰 ₹{plan['price']}\n⏳ {plan['validity']} days\n"
                       f"💳 UPI: {upi}\n\nScan QR to pay")
            # Validate link before passing
            link = plan.get('link')
            if link and not (link.startswith('http://') or link.startswith('https://')):
                link = None  # invalid, so don't pass
                logger.warning(f"Invalid link for plan {plan_id}: {plan.get('link')}")
