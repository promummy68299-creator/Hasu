...
# QR dependencies – must be installed
import qrcode
from PIL import Image
import urllib.parse

# ---------- CONFIG ----------
BOT_TOKEN = "8710908239:AAGGboJT01MsT1YRXSIh4zP5pKwXwCCIzgo"   # Replace with your bot token
ADMIN_ID = 7709767483                # Replace with your Telegram user ID (integer)

if BOT_TOKEN == "YOUR_BOT_TOKEN_HERE":
    print("❌ Please set BOT_TOKEN in bot.py")
    sys.exit(1)
...
def add_plan(name, price, validity, link, image_file_id='', description=''):
    conn = get_db()
    c = conn.cursor()
    c.execute('INSERT INTO plans (name, price, validity, link, image_file_id, description) VALUES (?, ?, ?, ?, ?, ?)',
              (name, price, validity, link, image_file_id, description))
    pid = c.lastrowid
    conn.commit()
    conn.close()
    return pid
...
# ---------- QR GENERATOR (SIMPLE, WITH LOGGING) ----------
def normalize_price(price) -> str:
    """Convert price to clean string for UPI QR."""
    if price is None:
    ...
    if isinstance(price, str):
        ...
        # Convert to float then int if needed
        try:
            price = float(price)
        except ValueError:
    ...
    if isinstance(price, int) or isinstance(price, float):
        ...
        # For UPI, we can use int if no decimals, else string with decimals
        if price == int(price):
...
def generate_upi_qr(upi_id: str, price, plan_name: str = "Premium") -> io.BytesIO:
    """
    Generate UPI QR code as BytesIO buffer.
    Raises exception with detailed error messages.
    """
    # Validate inputs
    ...
    logger.info(f"QR generation started: price={price_str}, upi={upi_id[:4]}...")

    # Build UPI URI with urlencode
    params = {
        "pa": upi_id.strip(),
        "pn": plan_name[:50],  # Telegram limit, but UPI allows longer
        "am": price_str,
        "cu": "INR"
    }
    upi_uri = "upi://pay?" + urlencode(params)
    logger.info(f"UPI URI: {upi_uri[:60]}...")
...
user_data = {}  # temporary storage for admin flows
...
def user_plans_keyboard(plans):
    ...
    for p in plans:
        ...
        kb.add(types.InlineKeyboardButton(label, callback_data=f"select_plan_{p['id']}"))
...
def plan_edit_keyboard(plan_id):
    ...
    kb.add(types.InlineKeyboardButton("🖼 Image", callback_data=f"edit_image_{plan_id}"))
...
def admin_welcome_keyboard():
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(types.InlineKeyboardButton("🖼 Set Image", callback_data="admin_set_welcome_image"))
    kb.add(types.InlineKeyboardButton("✏️ Set Text", callback_data="admin_set_welcome_text"))
    kb.add(types.InlineKeyboardButton("👁 Preview", callback_data="admin_preview_welcome"))
    kb.add(types.InlineKeyboardButton("🗑 Remove Image", callback_data="admin_remove_welcome_image"))
    kb.add(types.InlineKeyboardButton("⬅️ Back", callback_data="admin_back"))
    return kb
...
def handle_callback(call):
    ...
    if data == "get_premium":
        ...
        if not upi:
            bot.edit_message_text("⚠️ Payment is currently unavailable. Please contact admin.",
                                  chat_id=call.message.chat.id, message_id=call.message.message_id)
            bot.answer_callback_query(call.id)
            return
        try:
            logger.info(f"QR generation started | plan_id={plan_id} | price={plan['price']} | upi_configured={bool(upi)}")
            # Normalize price for QR
            qr_buffer = generate_upi_qr(upi, plan['price'], plan['name'])
            # Send plan image if exists
            if plan.get('image_file_id'):
                bot.send_photo(user_id, plan['image_file_id'], caption=f"💎 {plan['name']}")
            # Send QR with details
            caption = (f"📦 Plan: {plan['name']}\n💰 ₹{plan['price']}\n⏳ {plan['validity']} days\n"
                       f"💳 UPI: {upi}\n\nScan QR to pay")
            bot.send_photo(user_id, qr_buffer, caption=caption,
                           reply_markup=plan_payment_keyboard(plan_id, plan.get('link')))
            bot.delete_message(call.message.chat.id, call.message.message_id)
            bot.answer_callback_query(call.id, "QR generated ✅")
            logger.info("QR sent successfully")
        except Exception as e:
            logger.exception(f"QR generation failed for plan_id={plan_id}")
            # Show a clear error to user, but still log the traceback
            bot.send_message(user_id, "❌ QR generation failed.\nPlease contact admin.")
    ...
    if data == "admin_welcome":
        ...
        bot.edit_message_text("📢 Send the message/media to broadcast to all users.",
                              chat_id=call.message.chat.id, message_id=call.message.message_id)
        bot.answer_callback_query(call.id)
    ...
    # Welcome settings
        ...
        if img:
            ...
            bot.send_message(user_id, txt)
        bot.answer_callback_query(call.id)
    elif data == "admin_remove_welcome_image":
        set_setting('welcome_image', '')
        bot.answer_callback_query(call.id, "Image removed")
    ...
    # Plans
        ...
        plan_id = int(data.split("_")[2])
        user_data[user_id] = {'edit_plan': plan_id, 'field': 'image'}
        bot.edit_message_text("Send new image (photo):", chat_id=call.message.chat.id,
                              message_id=call.message.message_id)
        bot.answer_callback_query(call.id)
    ...
    # Demo
        ...
        bot.edit_message_text("Select to delete:", chat_id=call.message.chat.id, message_id=call.message.message_id,
                              reply_markup=kb)
        bot.answer_callback_query(call.id)
    elif data.startswith("del_demo_"):
    ...
    # Tutorial
        ...
        if video:
            ...
            bot.send_message(user_id, txt)
        bot.answer_callback_query(call.id)
    elif data == "admin_delete_tutorial":
...
def handle_text(message):
    ...
    if user_id == ADMIN_ID and user_data.get(user_id, {}).get('add_plan'):
        ...
        if step == 'name':
            ...
            bot.reply_to(message, "🖼 Send plan image (photo) or type 'skip':")
        elif step == 'image':
            # Image handled in photo handler
            if text.lower() == 'skip':
                image = ''
                ...
                add_plan(name, price, validity, link, image)
                bot.reply_to(message, f"✅ Plan '{name}' added!")
                del user_data[user_id]
            else:
                bot.reply_to(message, "Please send a photo or type 'skip'.")
        return
...
def handle_photo(message):
    ...
    # Admin set image
    if user_id == ADMIN_ID and user_data.get(user_id, {}).get('set') == 'welcome_image':
        set_setting('welcome_image', file_id)
        bot.reply_to(message, "✅ Welcome image updated")
        del user_data[user_id]['set']
        return

    # Admin add plan image
    if user_id == ADMIN_ID and user_data.get(user_id, {}).get('add_plan') and user_data[user_id].get('step') == 'image':
        name = user_data[user_id]['name']
        price = user_data[user_id]['price']
        validity = user_data[user_id]['validity']
        link = user_data[user_id]['link']
        add_plan(name, price, validity, link, file_id)
        bot.reply_to(message, f"✅ Plan '{name}' added with image!")
        del user_data[user_id]
        return

    # Admin edit plan image
    if user_id == ADMIN_ID and user_data.get(user_id, {}).get('edit_plan') and user_data[user_id].get('field') == 'image':
        plan_id = user_data[user_id]['edit_plan']
        update_plan(plan_id, image_file_id=file_id)
        bot.reply_to(message, "✅ Image updated")
        del user_data[user_id]
        return
    ...
    if user_id in user_data and 'payment_plan' in user_data[user_id]:
        ...
        # Notify admin
        admin_text = f"💳 New payment #{payment_id}\nUser: {message.from_user.username or message.from_user.first_name}\nPlan: {plan['name']} - ₹{plan['price']}"
